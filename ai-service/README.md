...
| 11 | 전량 측정 | ✅ 완료 · 90장 전량, 최종 결론 정확도 78.9% |

**파이프라인 11단계가 전 구간 동작한다.** 사진 90장 전량 확보·측정을 마쳤다.
남은 것은 성능 튜닝이다.

**STEP 1(`AI_MODE=mock`)은 모델 가중치도 API 키도 없이 동작한다.**
백엔드 담당자는 지금 바로 연동을 시작하면 된다.

### 실측 (동결 평가셋 90장 전량, CPU + Claude Sonnet 5)

지시서 §7.3의 7개 지표. `python scripts/evaluate.py` 로 재현한다.

| # | 지표 | 값 | 이전 (62장) |
|---|---|---|---|
| 1 | YOLO 검출률 (크롭 생성) | 78.2% (61/78) | 79.3% |
| 2 | YOLO 클래스 정확도 | 48.7% (38/78) — 참고 지표, VLM이 교정 | 50.0% |
| 3 | VLM 품목 정확도 | 73.3% (66/90) | 72.6% |
| 4 | VLM 상태 정확도 | 67.4% (60/89) | 71.2% |
| 5 | **최종 결론 정확도** | **78.9% (71/90)** | 77.4% |
| 6 | Before/After 판정 | 71.0% (22/31) | 68.0% |
| 7 | 평균 응답 시간 | **6.8초 / 장** | 6.8초 |

> 7번은 **캐시가 비어 있을 때의 값**이다. 위 측정은 캐시를 데운 뒤 돌려서
> 81/90이 캐시 히트였고 실측 평균이 250ms로 찍혔다. 그 숫자는 실제 지연이
> 아니므로 인용하지 않는다.

표본이 62장에서 90장으로 늘었는데 최종 정확도가 77.4% → 78.9%로 유지·소폭
상승했다. 컵라면 종이용기 규칙 수정(아래 참고)이 반영된 값이라 이전 표의
"재측정 전까지는 위 숫자가 공식 값" 단서는 해소됐다.

품목별 최종 결론 정확도:

| 품목 | 정확도 | | 품목 | 정확도 |
|---|---|---|---|---|
| 투명 페트병 | 12/12 · 100% | | 칫솔 | 3/4 · 75% |
| 알루미늄캔 | 9/10 · 90% | | 영수증 | 3/4 · 75% |
| 컵라면 종이용기 | 9/10 · 90% | | 아이스팩 | 3/4 · 75% |
| 택배상자 | 8/10 · 80% | | 건전지 | 3/4 · 75% |
| 배달 플라스틱 용기 | 8/10 · 80% | | 나무젓가락 | 3/4 · 75% |
| 아이스크림 포장지 | 6/10 · 60% | | 코팅지 | 2/4 · 50% |
| | | | 빨대 | 2/4 · 50% |

컵라면 종이용기가 50% → 90%로 올랐다. 규칙 수정이 실제로 들었다.

미검출 17장은 **VLM 단독 경로**로 라우팅된다. 실패가 아니라 설계된 폴백이다.

#### 남은 오답 19건의 원인

| 원인 | 건수 | 성격 |
|---|---|---|
| 품목 오분류 → 다른 배출함 안내 | 6 | VLM 판정 한계 |
| Before인데 배출 결론을 냄 | 6 | 준비 안내 대신 결론. 지시서 §4.3 위반 |
| `MULTIPLE_OBJECTS` 오탐 | 4 | 물건 하나인데 여럿으로 봄 |
| `IMAGE_TOO_BLURRY` | 2 | 실제로 흔들린 사진 |
| `NOT_WASTE` 오거부 | 2 | — |
| `CLASS_MISMATCH` | 1 | Before/After 품목이 클래스까지 달라짐 |

**Before 단계에서 배출 결론을 내는 6건이 새로 드러난 가장 큰 덩어리다.**
`box1 (1)` · `box5 (1)` 은 "준비 필요 안내"가 정답인데 곧장 `종이류`로,
`a1 (1)` · `a4 (1)` · `a5 (1)` 은 `비닐류`로 결론지었다. 아이 입장에서는
할 일을 안내받지 못하고 배출함만 통보받는 셈이다. (지시서 §4.3)

`MULTIPLE_OBJECTS` 오탐 4건도 늘었다. `receipt (3)` 은 영수증 한 장인데
여럿으로 잡혔다. 임계값 재조정 여지가 있다.

아이스크림 포장지(60%)는 "끈적임"을 VLM이 오염으로 보지 않아 준비 행동이 생기지 않는 경우다.
빨대·코팅지(각 50%)는 표본이 4장뿐이라 한 장이 12.5%p를 움직인다. 해석에 주의한다.
...
python scripts/evaluate.py                    # 전량. 90장 기준 약 $0.7 (캐시 없을 때)
...
"미리 준비해 둔 사진으로 다시 해볼래?"로 안내된다.

**현재 상태 — 평가셋 90장 중 81장이 데워져 있다** (`--eval-set` 실행, VLM 75회
호출, 스크립트 추정 약 $0.60). 오프라인 경로를 실제로 리허설해 확인했다.

| | 응답 | 결과 |
|---|---|---|
| 캐시에 있는 사진 | 511ms | 판정 정상 |
| 캐시에 없는 사진 | 325ms | `FAILED` / `VLM_UNAVAILABLE` + 아이용 안내문 |

데우지 못한 9장은 AI가 거부한 사진이라 저장할 판정 자체가 없다
(`MULTIPLE_OBJECTS` 4 · `IMAGE_TOO_BLURRY` 2 · `NOT_WASTE` 2 · `CLASS_MISMATCH` 1).
**시연에는 쓰지 않는다.**

> ⚠️ 프롬프트나 배출 규칙을 고치면 이 캐시는 통째로 무효다. 캐시는 사진 내용으로
> 키를 잡으므로 고친 뒤에도 옛 판정이 되돌아온다. `--clear` 후 다시 데워야 하고
> 비용도 다시 든다.
