...
**파이프라인 11단계가 전 구간 동작한다.** 남은 것은 성능 튜닝과 나머지 사진이다.
...
python scripts/evaluate.py                    # 전량. 110장 기준 약 $1
...
- 항목 하나가 **1.5KB**다. 110장을 다 데워도 200KB 남짓이다.
...
평가셋 110장 전량을 Sonnet으로 돌려도 **약 $1**이라 비용이 제약이 아니다.
...
| 촬영 사진 110장 | `eval_frozen/images/` | 🔶 62장 확보 (git 제외) |
